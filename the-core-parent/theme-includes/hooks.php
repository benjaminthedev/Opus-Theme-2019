...
if (!function_exists('_the_core_action_shortcode_latest_posts_enqueue_dynamic_css')) :
	...
	function _the_core_action_shortcode_latest_posts_enqueue_dynamic_css($data)
	{
		...
		if (isset($atts['blog_title']['selected']) && !empty($atts['blog_title']['selected'])) {
			...
			// hover color
			$title_hover_color = the_core_get_color_palette_color_and_class($atts['blog_title'][$h]['advanced_styling']['hover_color'], array('return_color' => true));
			if( !empty($title_hover_color['color']) ) {
				$final_styles .= '.tf-sh-' . $atts['unique_id'] . ' .post ' . $h . '.entry-title a:hover { color:' . $title_hover_color['color'] . ' !important;}';
			}
...
if ( !function_exists('_the_core_action_shortcode_fw_custom_posts_enqueue_dynamic_css') ) :
	/**
	 * @internal
	 *
	 * @param array $data
	 */
	function _the_core_action_shortcode_fw_custom_posts_enqueue_dynamic_css($data) {
		$shortcode = 'fw_custom_posts';
		$atts = shortcode_parse_atts($data['atts_string']);
		...
		// description styling
		$description_advanced_styling = the_core_get_shortcode_advanced_styles($atts['description_advanced_styling']['description'], array('return_color' => true));
		if (!empty($description_advanced_styling['styles'])) $final_styles .= '.tf-sh-' . $atts['unique_id'] . ' .post .entry-content {' . $description_advanced_styling['styles'] . '}';

		// grid bg color
		$grid_bg_color = the_core_get_color_palette_color_and_class($atts['grid_bg_color'], array('return_color' => true));
		if ( !empty($grid_bg_color['color']) ) {
			$final_styles .= '.tf-sh-' . $atts['unique_id'] . '.postlist-grid .post .entry-header, .tf-sh-' . $atts['unique_id'] . '.postlist-grid .post .entry-content { background-color: ' . $grid_bg_color['color'] . '; }';
			$final_styles .= '.tf-sh-' . $atts['unique_id'] . '.postlist-grid .post .entry-header, .tf-sh-' . $atts['unique_id'] . '.postlist-grid .post .entry-content { padding-left: 20px; padding-right: 20px; }';
			$final_styles .= '.tf-sh-' . $atts['unique_id'] . '.postlist-grid .post .entry-content { padding-bottom: 20px; padding-top: 10px; }';
		}
...
add_action('fw_ext_shortcodes_enqueue_static:fw_custom_posts', '_the_core_action_shortcode_fw_custom_posts_enqueue_dynamic_css');
...
if ( ! function_exists( 'the_core_filter_show_404_page' ) ):
	 ...
	 */
	function the_core_filter_show_404_page( $original_404 ) {
...
if ( ! function_exists( 'the_core_action_before_close_main_theme' ) ) :
	function the_core_action_before_close_main_theme() {
		$page_404 = function_exists( 'fw_get_db_settings_option' ) ? fw_get_db_settings_option( 'page_404' ) : '';
		if ( ! empty( $page_404 ) && is_404() && function_exists( 'wpml_object_id_filter' ) ) {
			global $post;
...
if ( ! function_exists( 'the_core_action_set_page_404' ) ) :
	/**
	 * 404 page set page_id
	 */
	function the_core_action_set_page_404() {
		if( is_admin() ) {
			return;
		}

		$page_404 = function_exists( 'fw_get_db_settings_option' ) ? fw_get_db_settings_option( 'page_404' ) : '';
		if ( ! empty( $page_404 ) && is_404() ) {
			$_REQUEST['page_id'] = function_exists( 'wpml_object_id_filter' ) ? wpml_object_id_filter( $page_404 ) : $page_404;
		}
	}
endif;
add_action( 'wp', 'the_core_action_set_page_404', 1 );


if ( ! function_exists( 'the_core_action_coming_soon_page_id' ) ) :
	/**
	 * Coming soon set page_id
	 */
	function the_core_action_coming_soon_page_id() {
		if( is_admin() ) {
			return;
		}
		...
		if ( isset( $enable_coming_soon['selected'] ) && $enable_coming_soon['selected'] == 'yes' ) {
			...
			if ( ! is_user_logged_in() && $enable_coming_soon['yes']['coming_soon_page'] != '0' ) {
				...
				$_REQUEST['page_id'] = function_exists( 'wpml_object_id_filter' ) ? wpml_object_id_filter( $enable_coming_soon['yes']['coming_soon_page'] ) : $enable_coming_soon['yes']['coming_soon_page'];
			}
		}
	}
endif;
add_action( 'wp_loaded', 'the_core_action_coming_soon_page_id' );
...
add_action('wp', '_the_core_action_coming_soon_page', 12);
...
if ( ! function_exists( 'the_core_action_body_tracking_scripts' ) ) :
    /**
     * Display tracking scripts in head
     */
    function the_core_action_body_tracking_scripts() {
        if ( function_exists( 'fw_get_db_settings_option' ) ) {
            $tracking_scripts = fw_get_db_settings_option( 'tracking_scripts', '' );
            if ( ! empty( $tracking_scripts ) ) {
                foreach ( $tracking_scripts as $script ) {
                    if ( isset( $script['script'] ) && ! empty( $script['script'] ) ) {
                        if ( isset( $script['position'] ) && 'body' == $script['position'] ) {
                            echo( $script['script'] );
                        }
                    }
                }
...
add_action( 'the_core_action_after_body_open_tag', 'the_core_action_body_tracking_scripts' );
...
if ( ! function_exists( 'the_core_give_load_admin_styles' ) ) :
	function the_core_give_load_admin_styles() {
		...
		if ( function_exists('give_is_admin_page') && give_is_admin_page() ) {
			if ( (isset($_GET['post_type']) && $_GET['post_type'] == 'give_forms') || 'give_forms' == $post_type ) {
				$return = true;
			}
		}

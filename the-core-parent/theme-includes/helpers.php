	...
	function the_core_get_blog_view( $the_core_blog_view, $selector = 'class', $the_core_sidebar_position = null ) {
		if ( $the_core_blog_view == 'grid' ) {
			if ( $selector == 'id' ) {
				...
				$the_core_blog_masonry = '';
				if ( defined('FW') && is_archive() ) {
					$the_core_blog_masonry = 'fw-portfolio-masonry-'.fw_get_db_settings_option('posts_settings/masonry', 'on');
				}

				return ( $the_core_sidebar_position == null || $the_core_sidebar_position == 'full' ) ? 'clearfix fw-row postlist-grid postlist-grid-cols3 '.$the_core_blog_masonry : 'clearfix fw-row postlist-grid postlist-grid-cols2 '.$the_core_blog_masonry;
			}
		}
...
if ( ! function_exists( 'the_core_get_instagram_photos' ) ):
	...
	function the_core_get_instagram_photos( $username, $items = 9 ) {
		if ( false === ( $instagram = get_transient( 'instagram-photos-' . sanitize_title_with_dashes( $username ) . '-'. $items ) ) ) {
			...
			$connect = wp_remote_get( 'https://instagram.com/' . trim( $username ), $api_args );
...
if( !function_exists( 'the_core_get_post_types' ) ) :
	/**
	 * Return the post types
	 */
	function the_core_get_post_types() {
		$args = array(
			'public'             => true,
		...
		$result         = array();
		$result['none'] = esc_html__( 'None', 'the-core' );
